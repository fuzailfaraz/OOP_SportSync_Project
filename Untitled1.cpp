...
public:
    ...
    string getTeamFromDept(const string& d){
        ...
        if(dept=="CS") return "CS";
        else if(dept=="DS") return "DS";
        else if(dept=="AI") return "AI";
        else if(dept=="MS") return "MS";
        else if(dept=="SS") return "SS";
        else if(dept=="ME") return "ME";
        else if(dept=="EE") return "EE";
        else if(dept=="AE") return "AE";
        return "Unknown";
    }
    ...
    void getInfo() {
        ...
        while(cin>> sportChoice && sportChoice !=0){
            if(sportChoice >=1 && sportChoice<=6){
                ...
                cout<<"\nInvalid choice. Please select from 1 to 6."<<endl;
            }
        }
        cin.ignore();
    ...
    void login() override{
            ...
            switch(choice){
                ...
                for(Player& p: allPlayers){
                    if(p.getID() == loginID) {
                        // Properly set the team name
                        p.TeamName = p.getTeamFromDept(p.Dept);
                        
                        cout<<"\n=== Login Successful ===" << endl;
                        cout<<"Player Name: " << p.getPlayerName() << endl;
                        cout<<"Department: " << p.getDept() << endl;
                        cout<<"Team: " << p.getTeamName() << endl;
                        cout<<"========================" << endl;
                        
                        // Create a new ScheduleManager to ensure fresh data
                        ScheduleManager scheduleManager;
                        scheduleManager.viewMySchedule(p);
                        found = true;
                        break;
                    }
                }
                ...
                if(!found) {
                    cout<<"\nPlayer ID not found! Please register first."<<endl;
                }
    ...
    static Player fromLine(const string& line) {
        ...
        getline(ss, temp, ','); p.ID = stoi(trim(temp));
        getline(ss, p.PlayerName, ','); p.PlayerName = trim(p.PlayerName);
        getline(ss, temp, ','); p.PlayerAge = stoi(trim(temp));
        getline(ss, p.Gender, ','); p.Gender = trim(p.Gender);
        getline(ss, temp, ','); p.KitNo = stoi(trim(temp));
        getline(ss, p.Dept, ','); p.Dept = trim(p.Dept);
        getline(ss, sportsStr, ','); sportsStr = trim(sportsStr);

        // Parse sports
        if(!sportsStr.empty()) {
            stringstream sportSS(sportsStr);
            string sportName;
            while(getline(sportSS, sportName, ';')){
                if(!sportName.empty()) {
                    p.sport.push_back(trim(sportName));
                }
            }
        }

        // Set TeamName from Dept
        p.TeamName = p.getTeamFromDept(p.Dept);
        return p;
    }
...
void ScheduleManager::viewMySchedule(const Player& p) {
    ...
    for (int i = 0; i < schedule.size(); ++i) {
        ...
        // cout << "Checking: " << schedule[i].team1 << " vs " << schedule[i].team2 << " against " << team << endl;
        
        if (schedule[i].team1 == team || schedule[i].team2 == team) {
            schedule[i].display(matchCount);
            found = true;
            matchCount++;
        }
    }
    ...
    if (!found) {
        cout << "\nNo matches scheduled for your team (" << team << ").\n";
        cout << "Make sure your department is correctly registered.\n";
    } else {
        cout << "\nTotal matches for your team: " << matchCount << endl;
    }
    cout << "==========================================\n";
}
